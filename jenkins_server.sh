...
if [ $# -ne 0 -a $NOASK -ne 1 ]; then
    ...
    case $1 in
    -c|--create)
        [ "${2}" = "admin" -o -z "${2}" -o -z "${3}" ] && printf "${RED}ERROR: invalid username or password.${NC}\n" && exit 1
        [ `echo $2 | grep -c "username="` -ne 0 -a `echo $3 | grep -c "password="` -ne 0 ] && username=`echo $2 | cut -d \= -f2` && password1=`echo $3 | cut -d \= -f2` ||　print_usage
